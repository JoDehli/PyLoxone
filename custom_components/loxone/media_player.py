...
from .const import DOMAIN, SENDDOMAIN, DEFAULT_AUDIO_ZONE_V2_PLAY_STATE
from .helpers import (
    get_all,
        ...
        audioZone.update(
            {
                "hass": hass,
                "typ": "AudioZoneV2",
                "room": get_room_name_from_room_uuid(loxconfig, audioZone.get("room", "")),
...
class LoxoneAudioZoneV2(LoxoneEntity, MediaPlayerEntity):
    ...
    def __init__(self, **kwargs):
        ...
        self._state = self.playStateToMediaPlayerState(DEFAULT_AUDIO_ZONE_V2_PLAY_STATE)
        self._volume = 0
    ...
    async def event_handler(self, event):
        ...
        if self.states["playState"] in event.data:
            self._state = self.playStateToMediaPlayerState(event.data[self.states['playState']])
            should_update = True

        if should_update:
            self.async_schedule_update_ha_state()

    def playStateToMediaPlayerState(self, playState) -> MediaPlayerState:
        match playState:
            case 0:
            ...
            case _:
                _LOGGER.warning(f"Unknown playState:{playState}")
    ...
    async def async_set_volume_level(self, volume: float) -> None:
        ...
        self.hass.bus.async_fire(SENDDOMAIN, dict(uuid=self.uuidAction, value=f"volume/{volumeInt}"))
        self.async_schedule_update_ha_state()
