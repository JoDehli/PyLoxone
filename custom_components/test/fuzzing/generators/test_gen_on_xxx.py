...
generator_fuzzer = GeneratorFuzzer(value_pool_fuzzer, 3)
...
@pytest.mark.timeout(300)
def test_climate_LoxoneRoomControllerV2() -> None:
    logger.info("Start test of LoxoneRoomControllerV2 from climate.py.")
    ...
    # Generate fuzzed sequences
    generated_sequences = generator_fuzzer.fuzz(LoxoneRoomControllerV2, start_methods, max_sequence_length, num_sequences)
    
    # Run the sequences on LoxoneRoomControllerV2
    results = generator_runner.run(LoxoneRoomControllerV2, generated_sequences)
    ...
    logger.info("LoxoneRoomControllerV2 test finished.")
